...
exports.status = (req, res) => {
    try {
        ...
        Enquiry.findOne({ _id: id })
            .then(r_detail => {
                const record_detail = {};
                record_detail.status = 'pending';
                if (r_detail.status == 'pending') {
                    record_detail.status = 'success';
                }
                if (r_detail.status == 'success') {
                    record_detail.status = 'reject';
                }
                if (r_detail.status == 'reject') {
                    record_detail.status = 'pending';
                }

                Enquiry.updateOne({ _id: id }, { $set: record_detail }).then(num => {
                    if (num.ok == 1) {
